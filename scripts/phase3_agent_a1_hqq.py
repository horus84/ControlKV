...
def main():
    ...
    out_file = os.path.join(save_dir, "hqq_results.jsonl")
    ...
    for model_name in models:
        ...
        for b in bits:
            def make_hqq(b=b):
                return QuantizedCache(backend="hqq", config=model.config, nbits=b, residual_length=128)
            try:
                QuantizedCache(backend="hqq", config=model.config, nbits=b, residual_length=128)
                _run_cond(f"hqq_{b}bit", make_hqq)
            except Exception as e:
                print(f"Skipping hqq_{b}bit: {e}")
